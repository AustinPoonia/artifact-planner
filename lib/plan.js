 ...
 * of an artifact that group does not deliver. `assemblyFor` refuses it, because
 * that is the one place where both facts — what a group runs and what it was
 * sent — are in hand at once, so by the time a view arrives here the contradiction
 * has been reported by name and the instance is simply not in the set.
 *
 * It refuses rather than throws, and the distinction reaches this file even though
 * no code here changes: a throw meant a view never arrived at all, so this planner
 * either ran on a whole view or did not run. Now it runs on a view with one
 * instance withheld, which is the case it already handled — it takes the instance
 * set as given — but it means an absent instance here is not necessarily an absent
 * *op*, and anything that ever reasons backwards from this plan to what the network
 * signed has to read `refusals` too.
 *
 * ## Where this lives
 *

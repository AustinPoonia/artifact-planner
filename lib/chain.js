 ...
 * **that day has already happened once and was not detected** —
 * `platform:diagnostics` went to `3.0.0` and this row stayed at two, so a kind
 * declaring `^3.0.0` was refused by name on a device that publishes it. The
 * upgrade path is the rejected argument above: once both callers pass the
 * composed table from `ArtifactPatform/lib/capabilities.js`, this constant is
 * deleted and the duplication with it.
 *

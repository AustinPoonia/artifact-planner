...
const PLATFORM_VERSIONS = {
  ...
  // Two, ascending, and for the same reason the diagnostics row below gives:
  // `writeDocument` arrived as `1.1.0` and `1.0.0` stays alive because the
  // resolver takes the *first* satisfying entry, so the three shipped adapters on
  // `^1.0.0` are held to the shape without it. Dropping `1.0.0` here would hand
  // them the wider shape and quietly grant every adapter the ability to write a
  // document; dropping `1.1.0` would refuse a port that the kernel resolves.
  'platform:host': ['1.0.0', '1.1.0'],

...
const PLATFORM_VERSIONS = {
  ...
  // One entry at 1.1.0 rather than two. The change was additive — `respond`
  // gained an optional `headers` — so `^1.0.0` still admits it, and keeping a
  // 1.0.0 alive would only give a consumer a baseline that cannot make a
  // redirect. `platform-diagnostics` keeps its older ones deliberately because
  // theirs were not additive; this one is.
  'platform:listen': ['1.1.0'],

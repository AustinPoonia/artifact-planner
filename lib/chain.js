...
const NATIVE = {
  ...
  // Scoped by instance, like `@store` and unlike `@documents`. A request is
  // addressed to whoever is listening, so a shared queue would let one instance
  // answer a question asked of another — which is not a collision, it is one
  // artifact speaking as another.
  'platform:listen': ({ instance }) => `@listen:${instance}`,
 ...
 *
 * That is why `platform:diagnostics` still publishes `1.0.0` and `2.0.0` beside
 * `3.0.0`: retiring either would have turned every `^1.0.0` and `^2.0.0` port in
 * the fleet into an unchecked one silently. With this table a retirement is a
 * **refusal that names the range and the versions**, which is the answer an
 * operator can act on.
 *
 ...
 *
 * The optional third argument to `validate` was the way out, and **both callers now
 * take it**: `ArtifactPatform/lib/boot.js` passes
 * `capabilities.PLATFORM_VERSIONS` on the boot refusal path, and
 * `artifact-operator/lib/check.js` passes the same composed table through the
 * subpath `artifact-platform/lib/capabilities.js`. Neither consults this constant
 * any more.
 *
 * It stays anyway, and stays as the *default* rather than becoming required, for
 * one reason: a required parameter would have every un-updated caller — a script,
 * a suite, a repo not in this checkout — get `undefined` and have this module
 * report "I could not check" on every platform port of every real graph. An
 * optional argument makes an un-updated caller behave exactly as it does today,
 * which is the one property worth keeping on a path that refuses a device's boot.
 *
 * So the ceiling below is now **this default's** ceiling and not the fleet's. It
 * is reached only by a caller that has not been updated; the two that decide
 * whether a device comes up are not among them.
 *
 ...
 *
 * ponytail: this is still a copy of a fact that lives in seven other repositories,
 * so a capability that publishes a new version and does not move this row makes
 * this module **refuse a range the device would have accepted** — a false refusal,
 * which is a different and louder failure than the silent pass it replaces, but
 * still wrong. That day has already happened once and was not detected:
 * `platform:diagnostics` went to `3.0.0` and this row stayed at two, so a kind
 * declaring `^3.0.0` was refused by name on a device that publishes it.
 *
 * What changed is who is exposed to it. The two callers that can refuse a device's
 * boot or mislead an admin now pass the composed table, so a future drift here is
 * a defect in a fallback rather than an outage. Deleting the constant outright is
 * the end state and is deliberately not taken: see the paragraph above for why the
 * argument stays optional.
 *
 * Until then there **is** a local guard, and the sentence that used to sit here —
 * that nothing here can detect that day, and that the version comparison belongs
 * in `ArtifactPatform/test/chain.test.js` because that suite is the only place
 * that can see both lists — was wrong on the premise rather than on the
 * conclusion. Seeing a sibling's `DECLARATIONS` does not require depending on it:
 * `test/chain.test.js`'s last case *reads* `../platform-<segment>/lib/declaration.js`
 * off disk for every row in `NATIVE` and derives each published version from the
 * list that file freezes, in that file's own order. No `file:../` link, no
 * require of a capability package, so the inversion this header refuses is not
 * reintroduced. Its ceiling is that the siblings have to be checked out beside
 * this repo — in a lone-repo checkout the case prints `# NOT MEASURED` and
 * measures nothing, which is why it is a second guard beside the kernel's suite
...
const PLATFORM_VERSIONS = {
  ...
  // One again. `writeDocument` was `platform:host@1.1.0` for a day and is
  // `platform:documents@1.0.0` now: the first consumer that wanted it was not an
  // adapter, and binding this capability to write a page would have handed it
  // `writeCommand` as well. Nothing ever ported `^1.1.0`, so the row goes back
  // rather than keeping a version alive for compatibility nobody needs.
  'platform:host': ['1.0.0'],
  'platform:documents': ['1.0.0'],
  // One entry at the newest rather than three. Both changes were additive —
  // `respond` gained an optional `headers` in 1.1.0, and `next` began carrying
  // the request's `headers` in 1.2.0 — so `^1.0.0` still admits them, and
  // keeping the older ones alive would only give a consumer a baseline that
  // cannot make a redirect or read the country a proxy added.
  // `platform-diagnostics` keeps its older ones deliberately because theirs
  // were not additive; these are.
  'platform:listen': ['1.2.0'],
  'platform:documentation': ['1.0.0'],
  // Three, and the two older ones are alive on purpose. `platform-diagnostics`'
  ...
  //
  // This row said two while `platform-diagnostics/lib/declaration.js` published
  // three, and the cost was measured rather than reasoned about: a kind
  // declaring `platform:diagnostics ^3.0.0` came back `ok: false` with
  // `PLATFORM_VERSION_OUT_OF_RANGE`, `explain` printed "wants
  // platform:diagnostics ^3.0.0, and this runtime publishes 1.0.0, 2.0.0", and
  // `boot.js` throws on `!verdict.ok` — so a device whose runtime does publish
  // `3.0.0` refused to come up at all. That is the false refusal the ceiling
  // above predicted, and the case at the end of `test/chain.test.js` is the
  // guard that now catches the next one.
  //
  // Written as "the ceiling above" on purpose: the ledger check matches the
  // marker token anywhere in a file, so prose *about* a ceiling registers as a
  // second one and demands a row for something that does not exist.
  'platform:diagnostics': ['1.0.0', '2.0.0', '3.0.0'],
 ...
 * table and its ceiling. The two callers that matter — `ArtifactPatform/lib/boot.js`
 * and `artifact-operator/lib/check.js` — **do** pass the composed set out of
 * `ArtifactPatform/lib/capabilities.js`, so they are right on a build this package
 * was not compiled beside; the default is what a caller that cannot see it falls
 * back to, and it is a fallback rather than the normal path.
 *
